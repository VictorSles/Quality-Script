...
DEFAULT_TIMEOUT = 15
# Removidas as listas LINHAS e OKNOK.
LADOS = ["TOP", "BOT"] # Mantido, pois é uma entrada necessária para o processo.
INJET_RESOURCES = [
...
# =======================
# API WIP INFO (Sem alterações significativas)
# =======================
...
# =======================
# EXTRAÇÃO & LOG (MODIFICADA PARA USAR PATHS DINÂMICOS E INFERIR STATUS)
# =======================
def extract_info(session, serial):
    # (Conteúdo de extract_info sem alteração de escopo)
    info = {}
...
def format_info_line(info, linha_api, lado, serial_oknok, defects):
    ...
    parts.append(f"Linha: {linha_api}") # Usa o valor da API
    parts.append(f"Lado: {lado}")
    parts.append(f"Status: {serial_oknok}") # Usa o status inferido
    if defects:
        nomes = "; ".join(d.get("DefectName", "") for d in defects)
        crds = "; ".join(d.get("Crd", "") for d in defects)
        status = "; ".join(d.get("DefectStatus", "") for d in defects)
...
# Modificada a assinatura e lógica para inferir oknok
def append_to_log(session, wip_data, linha_manual_unused, lado, oknok_manual_unused, log_file=None, network_path=None):
    ...
    # 💡 Lógica para inferir o status (OK/NOK)
    serial_oknok = "OK" 
    ...
    # Obtém a linha (Manufacturing Area) da API
    linha_api = ""
    if lado == "TOP":
...
# =======================
# TKINTER APP (MODIFICADA PARA REMOVER BOTÕES DE LINHA E STATUS)
# =======================
class WipApp(tk.Tk):
    ...
    def process_queue(self):
        ...
        # Apenas o 'lado' é necessário
        lado = self.lado_var.get()

        for serial in seriais:
            # Chama process_serial apenas com o serial e lado
            threading.Thread(target=self.process_serial, args=(serial, lado), daemon=True).start()
    ...
    def create_widgets(self):
        ...
        # 💡 FRAME DE OPÇÕES SIMPLIFICADO: APENAS LADO
        frame_opcoes = ttk.Frame(self)
        frame_opcoes.pack(pady=(10, 10))
        
        ttk.Label(frame_opcoes, text="Lado:").grid(row=0, column=0, padx=5, sticky="e")
        self.lado_var = tk.StringVar(value=LADOS[0])
        ttk.Combobox(frame_opcoes, textvariable=self.lado_var, state="readonly", values=LADOS, width=10).grid(row=0, column=1, padx=5)
    ...
    # 💡 Assinatura da função alterada (removido 'linha' e 'oknok')
    def process_serial(self, serial, lado):
        ...
        # 💡 Chamadas atualizadas: "N/A" para 'linha' e None para 'oknok' (que são ignorados dentro da função)
    ...
    def process_panel(self, session, serial_principal, lado):
        ...
        # 💡 Chamada atualizada: "N/A" para 'linha' e None para 'oknok'
