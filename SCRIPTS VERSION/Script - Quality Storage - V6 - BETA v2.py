...
# =======================
# EXTRAÇÃO & LOG (MODIFICADA PARA USAR PATHS DINÂMICOS)
# =======================
...
def append_to_log(session, wip_data, linha, lado, oknok=None, log_file=None, network_path=None):
    if not log_file or not network_path:
        return 0 # Não pode prosseguir sem o caminho
...
def append_to_log_all_boards(session, panel_data, linha, lado, oknok=None, serial_principal=None, log_file_all=None, network_path=None):
...
# =======================
# EXPORTAÇÃO EXCEL (MODIFICADA PARA USAR PATHS DINÂMICOS)
# =======================
...
class WipApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # 💡 Inicializa as variáveis de caminho
        ...
        # 💡 Define os caminhos iniciais após criar o label
    ...
    # 💡 Função central para definir todos os caminhos de arquivo
    ...
    # 💡 Função para atualizar o label de status do caminho
    def update_path_label(self):
        if hasattr(self, 'path_label'):
            self.path_label.config(text=f"Pasta de Salvamento: {self.network_path}")

    # 💡 Função para definir o caminho pré-definido
    ...
    # 💡 Função para selecionar um caminho personalizado
    ...
    def create_widgets(self):
        ...
        # 💡 O comando de exportar agora usa as variáveis de instância
        ttk.Button(
            frame_botoes,
            text="📤 Exportar Excel",
            width=18,
    ...
    def process_serial(self, serial, linha, lado, oknok=None):
        ...
        # 💡 Passa os paths de log e de rede para as funções
        cont = append_to_log(self.session, wip, linha, lado, oknok, self.log_file, self.network_path)
    ...
    def process_panel(self, session, serial_principal, linha, lado, oknok=None):
        ...
        # 💡 Usa a função de gravação com os paths de instância
        return append_to_log_all_boards(
            session, panel_data, linha, lado, oknok, serial_principal, self.log_file_all, self.network_path
        )
