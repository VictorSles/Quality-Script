...
def format_info_line(info, linha_api, lado, serial_oknok, defects, oba_user):
    ...
    parts.append(f"OBA: {oba_user}") # 💡 NOVO CAMPO OBA
...
def export_from_txt_to_excel(log_files_list, excel_path):
    ...
    # 💡 Tratamento para a nova coluna 'OBA'
    if "OBA" not in expanded_df.columns:
         expanded_df["OBA"] = ""
    if "Turno" not in expanded_df.columns:
         expanded_df["Turno"] = ""

    # Reordenar colunas para colocar OBA em uma posição lógica (ex: antes de DataHoraProcessamento)
    cols = list(expanded_df.columns)
    cols_to_move = ["OBA", "Turno"]
    
    # Remove e insere as colunas no local desejado
...
class WipApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Esconde a janela principal para mostrar o diálogo de login primeiro
        self.withdraw()
        
        # 💡 Novas variáveis de instância para credenciais e OBA
        ...
        # 💡 Constrói a sessão com as credenciais fornecidas
        ...
        self.deiconify() # Revela a janela principal após o login
    ...
    def prompt_for_credentials(self):
        ...
        try:
            # Pede o nome de usuário com "jabil\" pré-escrito
        ...
        # Extrai o nome de usuário (OBA) sem o domínio
    ...
    def process_serial(self, serial):
        ...
        # 💡 process_panel irá usar o self.oba_user
        ...
        if cont == 0:
            ...
            # 💡 Passa self.oba_user para o resumo
